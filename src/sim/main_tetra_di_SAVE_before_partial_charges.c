...
char out_folder[] =  "/usr/people/jwalther/Programs/Chromatin/src_test";   //"/scratch/orozco/jwalther/chromatin/200mer/random_seq5/test/4";	

char seq_file[] = "rand_seq5_200.dat";							
char stif_file[] = "stif_150mer.dat";
int ins_nucl[] = {19,39,59,79,99,119,139,159,179};  	// Because 0th bp is not counted (like this 1st and last linker have same length)
...
// Test 300mer: 20,40,60,80,100,120,140,220,240,260,280
...
double **d_nucl, **d_nucl_bp, **d_nucl_fl;
...
lj_bp = dbl_aloc2d(bp_all*bp_all,no_helpars);
lj_bp_fl = dbl_aloc2d(bp_all*bp_all,no_helpars);
...
dh_ph = dbl_aloc2d(2*2*bp_all*2*bp_all,no_helpars);
...
acc = 10;			// bp-steps for calculating distance (usually 10)
acc_ph = 15;			// phosphate = bp steps for calculating distance (usually 20)
...
/* ---- Parameters for LJ ---- */	
lj_k = 4*0.0257; 		// in eV (0.0257 eV = 1kT, value 4*0.0257  adapted from Arya 2014)
lj_cut_up = 30;		// 1.5*bp_excl_vol (= 30)
lj_cut_low = 0.1;
...
printf("\n");*/
	...
	nucl_dist(bp_all,no_nucl,num_fl,itot,acc,center_bp,nucl_pos,ins_nucl,pos_old,d_nucl,d_nucl_bp,d_nucl_fl,xyz_float);
	...
	if(iflag == 1){if(psi%1 == 0){printf("Total energy LJ + DH + elastic = %lf + %lf + %lf = %lf kcal/mol\n",tot_lj*23,tot_dh*23,endenergy*23,tot_en);
