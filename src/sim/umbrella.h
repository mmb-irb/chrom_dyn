...
void copy_1darray(double *a, double *b, int n)
{
int i,j;
for(i=0;i<n;i++){
		a[i] = b[i];
...
void transfer_pos_and_orient(int itt, int itot, double ***store_orient_t_i, double **store_r_mst, double ***smart_t_i, double **smart_r_i)
{
int i,j,k;
...
void transfer_R(int itt, int itot, double ***store_orient_t_i, double ***store_r_mst, double ***smart_t_i, double ***smart_r_i)
{
int i,j,k;
...
for(i=itt;i<itot;i++){
			...
			for(j=0;j<3;j++){
			for(k=0;k<3;k++){
					store_r_mst[i][j][k] = smart_r_i[i][j][k];
					}	
					}
...
void transfer_ph_pos(int itt, int itot, double **buf_pos_ph, double **pos_ph)
{
int i,j;

for(i=itt;i<itot;i++){
			for(j=0;j<3;j++){
					buf_pos_ph[i][j] = pos_ph[i][j];
					}	
...
void cart_rec_old(int itot, int no_rot_pars, double **xconf, double **store_r_mst, double ***store_orient_t_i, double ***store_R_T_mst, double ***store_R_T_i)
{
int i,j,k,m;
const float pi = 3.14159265358979323846;
double **rotz1, **roty, **rotz2;
...
end_r_mst = malloc(no_rot_pars*sizeof(double));
	...
	for(i=0;i<itot;i++){	// result is multiplication of all
			...
			copy_2darray(store_R_T_mst[i],res_buf2,3);	// Store rot matrix for mid-step triad
			...
			copy_2darray(store_R_T_i[i],res_buf2,3);	// Store rot matrix for T_i+1
...
/* Adapted from Felipe */
void end_to_end_dist_fast(int itot, int i0, int no_rot_pars, double **xconf, double **store_r_mst, double ***store_orient_t_i, double ***store_R_T_mst, double ***store_R_T_i)
{
	int i,j,k,I;
	const float pi = 3.14159265358979323846;
	double **rotz1, **roty, **rotz2;
	...
	double **res_buf1, **res_buf2, **result, **res_prev;
	double *transl, **t_mst;
	double *r_mst, *r_bppar, *end_r_mst;
	double ete_dist_buf;
	//double eps = 1e-10;
	
	//if (i0 > i1) {i0 = 0; i1 = itot;}
	rotz1 = dbl_aloc2d(no_rot_pars,no_rot_pars);
	rotz2 = dbl_aloc2d(no_rot_pars,no_rot_pars);
	roty = dbl_aloc2d(no_rot_pars,no_rot_pars);
	res_buf1 = dbl_aloc2d(no_rot_pars,no_rot_pars);
	...
	if (i0 != 0) {
	  for (i=0;i<3;i++)
	    for(j=0;j<3;j++) res_prev[i][j] = store_orient_t_i[i0-1][i][j];
	    
	  for(i=0;i<3;i++){end_r_mst[i] = store_r_mst[i0-1][i];}
	}

  

	//Calculate everything new for i=itt (=i0 in this code)
    //i0 = i0%itot;
	p = atan(xconf[3][i0]/xconf[4][i0]); //in rad (mixture of tilt and roll from Hassan&Calladine 95)
		g = xconf[4][i0]/cos(p)*pi/180;	//in rad (mixture of tilt and roll from Hassan&Calladine 95)
		tw = xconf[5][i0]*pi/180;	//in rad
		...
		copy_2darray(store_R_T_mst[i0],res_buf2,3);	// Store rot matrix for mid-step triad
		...
		r_bppar[0] = xconf[0][i0];	//shift
		r_bppar[1] = xconf[1][i0];	//slide	
		r_bppar[2] = xconf[2][i0];	//rise
		...
		copy_2darray(store_R_T_i[i0],res_buf2,3);	// Store rot matrix for T_i+1
		...
		for(j=0;j<3;j++){
		  for(k=0;k<3;k++){
				store_orient_t_i[i0][j][k] = res_prev[j][k]; // store orientation matrix T_i+1
			}
		}



	for(i=i0+1;i<itot;i++){	// result is multiplication of all
	  I = i;
   // i = i%itot;
			...
			for(j=0;j<3;j++){
			for(k=0;k<3;k++){
					res_buf2[j][k] = store_R_T_mst[i][j][k]; // store rotation matrix for calculating T_mst
					}
					}
		...
		for(j=0;j<3;j++){
			...
			for(j=0;j<3;j++){
			for(k=0;k<3;k++){
					res_buf2[j][k] = store_R_T_i[i][j][k]; // store rotation matrix for calculating T_mst
					}
					}
	...
	//for(i=0;i<3;i++){printf("%lf\n",store_r_mst[1][i],store_r_mst[2][i],store_r_mst[itot-2][i]);}
...
void end_to_end_dist_fast_orig_felipe(int itot, int i0, int i1, int no_rot_pars, double **xconf, double **store_r_mst, double ***store_orient_t_i)
{
	int i,j,k,I;
	const float pi = 3.14159265358979323846;
	double **rotz1, **roty, **rotz2;
	double g,p,tw;	//gamma,phi,twist (for end-to-end distance)
	double **res_buf1, **res_buf2, **result, **res_prev, *end_r_mst;
	double *transl, **t_mst;
	double *r_mst, *r_bppar;
	double ete_dist_buf;
	double eps = 1e-10;
  ...
  /*else {
	  for (i=0;i<3;i++){
	    for(j=0;j<3;j++){ res_prev[i][j] = store_orient_t_i[itot-1][i][j];}}	// Correct if Phantom bp works (right now it doesnt)
	    //unit_matrix3(res_prev);

	  for(i=0;i<3;i++){end_r_mst[i] = store_r_mst[itot-1][i];}			// Correct if Phantom bp works (right now it doesnt)
		//for(i=0;i<3;i++){end_r_mst[i]=0;}
	}*/
  
	for(i=i0;i<i1;i++){	// result is multiplication of all
	  I = i;
    i = i%itot;
