...
char out_folder[] =  "/usr/people/jwalther/Programs/Chromatin/src_test";   	//"/scratch/orozco/jwalther/chromatin/200mer/random_seq5/test/test_acc";

char seq_file[] = "rand_seq5_300.dat";							
char stif_file[] = "stif_150mer.dat";
int ins_nucl[] = {19,39,59,79,99,119,139,159,179,199,219,239,259,279};  	// Because 0th bp is not counted (like this 1st and last linker have same length)
//160,180,200,240,260,280 ,160,180,200,220,240,260,280,300,320,340,360,380,400,420,440,460,480,500,520,540,560,580 20  40  60  80 100 120 140 160 180 200 220 240 260 280 300 320 340 360 380 400 420 440 460 480 500 520 540 560 580
...
red_bp_all = (int) (double)bp_all/(double)acc;
printf("red_bp_all %d\n",red_bp_all);
...
lj_bp = dbl_aloc2d(bp_all*bp_all,no_helpars);	printf("Test\n");	
lj_bp_fl = dbl_aloc2d(bp_all*bp_all,no_helpars);printf("Test1\n");
...
dh_ph = dbl_aloc2d(2*2*bp_all*2*bp_all,no_helpars); printf("Test2\n");	// GIVES SEGMENTATION FAULT IN GPU CLUSTER!!!
dh_ph_fl = dbl_aloc2d(2*bp_all*bp_all,no_helpars);
...
d_nucl_bp = dbl_aloc2d(no_nucl,bp_all);
d_nucl_fl = dbl_aloc2d(no_nucl,bp_all);
d_nucl_ph = dbl_aloc2d(no_nucl,2*len_all);

table_bp  = char_aloc2d(bp_all,len_out_line);
table_bp_triad  = char_aloc2d(4*bp_all,len_out_line);
...
lj_nucl = dbl_aloc2d(no_nucl*no_nucl,4);
lj_nucl_bp =  dbl_aloc2d(no_nucl*bp_all,4);
lj_nucl_fl =  dbl_aloc2d(no_nucl*bp_all,4);

in_q_ph_ph = int_aloc2d(2*len_all,2*len_all);
in_q_nucl_ph = int_aloc2d(no_nucl,2*len_all);
q_nucl = malloc(no_nucl*sizeof(double));

dh_nucl = dbl_aloc2d(no_nucl*no_nucl,4);
dh_nucl_ph =  dbl_aloc2d(no_nucl*2*len_all,4);
dh_nucl_fl =  dbl_aloc2d(no_nucl*len_all,4);
...
it_mc = 2*it_mcsteps;		// number of MC steps to bring structure to minimum energy before particle inserted
				/* 100000 -> 56mer
				   10000000 -> 1000mer
				   2000000 -> 450mer
				   5000000 -> 750mer
				*/
metr_on = 1;			// 1 = DNA gets sampled with metropolis, 2 = all MC moves get accepted
...
rnd_start = 2;			// Use random starting structure (1) or use xconfi as starting structure
...
/* ---- Parameters for LJ ---- */	
lj_k = 1*0.0257; 		// in eV (0.0257 eV = 1kT, value 1*0.0257  adapted from Arya 2014)
...
/* ---- Parameters for DH ---- */
eps = 80.0;
kappa = 0.033; 			// unit: 1/A	taken from Arya 2014 paper (usually 0.033)
...
center_nucl[0] = 15.11286;	// Derived from the mean of bp positions of 1kx5 structure
center_nucl[1] = -35.92884;
center_nucl[2] = 42.59748;

nucl_nucl_excl_vol = 110;	// 2*5.5nm (since nucleosome is 11nm in diameter)	(DNA in nucleosome still considered seperately)
nucl_bp_excl_vol = 65;		// minimum distance between nucleosome and center of bp (55A+10A)
...
for(i=0;i<no_nucl;i++){q_nucl[i] = 94/acc_ph;}	// charge histone core (2*146*q_nucl_dna + q_nucl = -198 = total charge of nucl core particle)
...
printf("\n");*/
	...
	if(psi == 0 && rnd_start ==1)
			...
			int buf_metr_on = 1;
			monte_carlo(buf_it_mcsteps,itot_mc,enertot,ener,xconft_mc,xconf_mc,sc,xconf0_mc,stiff_mc,enex,dd,boltz,temp,iflag, &endenergy, buf_metr_on, &no_mc_move);
			no_mc_move = 0;
		     }
	else{
	// ---- Normal execution
	monte_carlo_ind(it_mcsteps,itot_mc,enertot,ener,xconft_mc,xconf_mc,sc,xconf0_mc,stiff_mc,enex,dd,boltz,temp,iflag, &endenergy, metr_on, &get_itt);
	...
	//for(j=0;j<200;j++){for(i=0;i<3;i++){printf("%lf ",pos_old[j][i]);} printf("\n");} printf("\n");
	...
	check_olap(bp_all,num_fl,len_all,acc,center_bp,pos_old,d_bp,d_bp_fl,xyz_float,bp_excl_vol,bp_fl_excl_vol, no_nucl,d_nucl,d_nucl_bp,d_nucl_fl,nucl_nucl_excl_vol, nucl_bp_excl_vol,nucl_fl_excl_vol,&vol_olap_start);
	...
	dist_ph(bp_all,num_fl,acc_ph,pos_ph,d_ph1,d_ph2,d_ph_fl,xyz_float,no_nucl,ins_nucl,d_nucl_ph,nucl_pos,in_q_ph_ph,in_q_nucl_ph,cut_d_ph_nucl,&m_nucl_ph);
	...
	calc_lj_nucl(&k5,&k6,&k7,bp_all,no_nucl,d_nucl,d_nucl_bp,d_nucl_fl,lj_nucl_cut_low,lj_nucl_cut_up,lj_nucl,lj_nucl_bp,lj_nucl_fl,lj_k_nucl,lj_k_nucl_bp, lj_k_nucl_fl,lj_a_nucl,lj_a_bp_nucl,acc,num_fl,lj_a_nucl_fl,&e_nucl_lj);
	...
	calc_dh_nucl(&k8,&k9,&k10,bp_all,no_nucl,d_nucl,d_nucl_ph,d_nucl_fl,dh_nucl_cut_low,dh_nucl_cut_up,dh_nucl,dh_nucl_ph,dh_nucl_fl,q_nucl, arr_q_ph,q_fl,in_q_nucl_ph,acc_ph,num_fl,eps,kappa,f_pi_eps0,&e_nucl_dh);
	...
	calc_dh(&k3,&k4,bp_all,dh_ph,d_ph1,d_ph2,dh_cut_low,dh_cut_up,arr_q_ph,eps,kappa,f_pi_eps0,acc_ph,num_fl,dh_ph_fl,d_ph_fl,q_fl,in_q_ph_ph);
	...
	tot_en = tot_dh_lj + endenergy;			// Energy DH+LJ DNA + elastic DNA (in eV)
	...
	metropolis(boltz/23,temp,buf_tot_en,tot_en,&iflag);	// Metropolis for total energy
	...
	if(iflag == 1){if(psi%1 == 0){printf("Total energy LJ (DNA) + LJ (nuc) + DH (DNA) + DH (nuc) + elastic = %.3f + %.3f + %.3f + %.3f + %.3f = %.3f kcal/mol\n",tot_lj,e_nucl_lj,tot_dh,e_nucl_dh,endenergy,tot_en);
	...
	CEHS_build(len_all,ist_all,xconf_all,1,output,out_folder);


	/* ----- Output positions and triads ----- */
	write_bp_pos_in_table(table_bp,len_all,phi,pos_old,out_folder);
	write_bp_triad_in_table(table_bp_triad,len_all,phi,orient_old,out_folder);
