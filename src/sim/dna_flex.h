...
int ***int_aloc3d(int x,int y,int z)
{
    int i,j;
...
void free_int3d(int ***array3D,int x)
{
  int i;
    for(i = 0; i < x; i++)
    {
...
void init_dbl2d_arr(double **a, int a1, int a2)
{
int i,j;
for(i=0;i<a1;i++){for(j=0;j<a2;j++){a[i][j] = 0;}}
}

void init_dbl3d_arr(double ***a, int a1, int a2, int a3)
{
int i,j,k;
for(i=0;i<a1;i++){for(j=0;j<a2;j++){for(k=0;k<a3;k++){a[i][j][k] = 0;}}}
}

/* ------------------ My added functions ----------------------- */

void get_seq_len(int *pointer_len_seq, int *pointer_itot, char seq_file[200])
{
FILE *myfile;
  char line[10000000];
  int i;
...
void readsq_ind(int *pointer_itot,char **ist,char seq_file[200])
{
FILE *myfile;
  char line[10000000];
  int i;
...
void readsq_di(int *pointer_itot,char **ist,char seq_file[200])
{
FILE *myfile;
  char line[10000000];
  int i;
...
void readsq_tetra(int *pointer_itot,char **ist,char seq_file[200])
{
FILE *myfile;
  char line[10000000];
  int i;
...
void readsq_tetra_di(int *pointer_itot,char **ist,char seq_file[200])
{
FILE *myfile;
  char line[10000000];
  int i;
...
void readss_ind(double ***stif,char **seq1,double **geom,int ord[6],char seq_file[200],char stif_file[])
{
FILE *myfile;
FILE *myseq_to_file;
   int i,j,k;
...
void readini_mc(int itot,double **xconfi,char conf_file[200])
{
FILE *myfile;
   int i,j,k;
   double buffer;
...
void monte_carlo_seg(int it_mc, int start, int stop, double enertot, double *ener, double **xconft, double **xconf, double sc[6], double **xconf0, double ***stiff, double enex, double dd[6], double boltz, double temp, double *endenergy, int metr_on)
{
...
for(i0=0;i0<it_mc;i0++){
	...
	double xtot = (double) (stop-start)+1e-9;
	int itt = start + (int) (xtot*r);		// the step to change
	//printf("itt %d \n", itt);
...
void cmb_link_nucl_seg(double **xconf_mc, double **xconf_nucl, double **xconf_all, char **ist_mc, char **ist_nucl, char **ist_all, int ins_nucl[], int no_nucl, int len_bp_nucl, int len_seq_nucl, int itot_mc, int len_all, int nucl_ind, int *len_seg, int *ind_linker)
{
int i,j,k,l,k1,k2;
int buf,buf_mc,buf_nucl,buf_itt,bufbuf_mc,bufbuf;
//char ist_lin_nucl[4], ist_nucl_lin[4];
...
if(nucl_ind == no_nucl-1){
ind_linker[k] = buf_mc;
ind_linker[k+1] = itot_mc;
transfer_hel_coord_gen(buf,len_all,xconf_all,buf_mc,xconf_mc);	// for last linker part
transfer_seq_gen(buf,len_all,ist_all,buf_mc,ist_mc);
buf = len_all;
			 }
...
void write_bp_pos_in_table_seg(char **table_bp,int itot, int phi, double **pos_store, char out_folder[], int dyn)
{
...
if(dyn==-1){
	    sprintf(filename,"%s/output_cart/pre_cart_%.6d.xyz",out_folder,phi);
	   }
else{
	sprintf(filename,"%s/output_cart/pre_cart_%.6d_%.6d.xyz",out_folder,phi,dyn);
	}
...
void write_acc_mc_moves_in_table(int sel, int *acc_arr, int accepted,int it_mcsteps, int phi, int it_mc, int metr_on, int do_sim_ann, char stif_file[], char seq_file[200], char out_folder[])
{
...
void write_nucl_pos_in_table_seg(char **table_nucl,int no_nucl, int phi, double **nucl_pos, char out_folder[], int dyn)
{
...
if(dyn==-1){
	...
	sprintf(filename,"%s/output_nucl/pre_nucl_%.6d_%.6d.xyz",out_folder,phi,dyn);
	}
FILE *file_cart;
file_cart=fopen(filename, "w");
...
table_ener  = char_aloc2d(len_all+0,200);
...
void write_indep_in_table(int it_en, int phi, double **save_en, char out_folder[])
{
...
table_ener  = char_aloc2d(it_en+1,100);

sprintf(filename,"%s/output_schnarp/independence_%.6d.dat",out_folder,phi);
FILE *file_cart;
file_cart=fopen(filename, "w");

	k=0;
	sprintf(table_ener[k],"psi acc_mc tot_en sed_co rg\n");
	k++;
	for(i=0;i<it_en;i++){
			sprintf(table_ener[k], "%.3lf %.3lf %.5lf %.5lf %.5lf\n",save_en[i][0],save_en[i][1],save_en[i][2],save_en[i][3],save_en[i][4]);
			//printf("i0 %d\n",i);			
			k++;
			    }
...
void get_num_nucl(int *pointer_itot, char nucl_file[200])
{
FILE *myfile;
  char line[10000000];
  int i;
...
void get_nucl_pos(int len, int *ins_nucl, char nucl_file[200])
{
FILE *myfile;
  char line[10000000];
  int i,k;
...
void write_bp_nucl_pos_in_table(char **table_pos,int itot, int phi, double **pos_store, int no_nucl, double **nucl_pos, char out_folder[], int dyn)
{
...
if(dyn==-1){
	    sprintf(filename,"%s/output_pos/pos_%.6d.xyz",out_folder,phi);
	   }
else{
	sprintf(filename,"%s/output_pos/pos_%.6d_%.6d.xyz",out_folder,phi,dyn);
	}
...
//if(phi==0){k=0;}else{k=13*phi;}
	...
	k=0;
	for(i=0;i<3;i++){sprintf(table_pos[k], "%d\n",itot+1+no_nucl);}
	k++;
	for(i=0;i<3;i++){sprintf(table_pos[k], "generated by Juergen Walther\n");}
	k++;
	for(i=0;i<3;i++){sprintf(table_pos[k], "C %.3lf %.3lf %.3lf\n",zero,zero,zero);}
	k++;
	for(i=0;i<itot;i++){
			sprintf(table_pos[k], "C %.3lf %.3lf %.3lf\n",pos_store[i][0],pos_store[i][1],pos_store[i][2]);		
			k++;
			    }

	for(i=0;i<no_nucl;i++){
			sprintf(table_pos[k], "N %.3lf %.3lf %.3lf\n",nucl_pos[i][0],nucl_pos[i][1],nucl_pos[i][2]);		
			k++;
			    }

			
for(i=0;i<(itot+3+no_nucl);i++){		
fputs(table_pos[i],file_cart);}
...
void write_bp_nucl_pos_in_table_GRO(char **table_pos,int itot, int phi, double **pos_store, int no_nucl, double **nucl_pos, char out_folder[], int dyn)
{
...
if(dyn==-1){
	    sprintf(filename,"%s/output_pos/pos_%.6d.gro",out_folder,phi);
	   }
else{
	sprintf(filename,"%s/output_pos/pos_%.6d_%.6d.gro",out_folder,phi,dyn);
	}
...
//if(phi==0){k=0;}else{k=13*phi;}
	...
	k=0;
	sprintf(table_pos[k], "generated by Juergen Walther\n");
	k++;
	sprintf(table_pos[k], "%5d\n",itot+1+no_nucl);
	k++;
	sprintf(table_pos[k], "%5d%-5s%5s%5d%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f\n",index,struc,c,k-1,zero,zero,zero,zero,zero,zero);
	k++;
	for(i=0;i<itot;i++){
			sprintf(table_pos[k], "%5d%-5s%5s%5d%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f\n",index,struc,c,k-1,pos_store[i][0]/10,pos_store[i][1]/10,pos_store[i][2]/10,zero,zero,zero);		
			k++;
			    }

	for(i=0;i<no_nucl;i++){
			sprintf(table_pos[k], "%5d%-5s%5s%5d%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f\n",index,struc,n,k-1,nucl_pos[i][0]/10,nucl_pos[i][1]/10,nucl_pos[i][2]/10,zero,zero,zero);		
			k++;
			    }

	sprintf(table_pos[k], "2000.0 2000.0 2000.0\n");

			
for(i=0;i<(itot+4+no_nucl);i++){		
fputs(table_pos[i],file_cart);}
