...
file_cart=fopen(filename, "w");
	...
	k=0;
	sprintf(table_ener[k],"psi acc_mc tot_en sed_co \n");
	k++;
	for(i=0;i<it_en;i++){
			sprintf(table_ener[k], "%.3lf %.3lf %.5lf %.5lf\n",save_en[i][0],save_en[i][1],save_en[i][2],save_en[i][3]);
			k++;
			    }
...
void write_vol_olap_in_table(int it_en, int iter, int *save_en, char out_folder[])
{
...
sprintf(filename,"%s/output_schnarp/vol_olap_%.6d.dat",out_folder,iter);
FILE *file_cart;
file_cart=fopen(filename, "w");
