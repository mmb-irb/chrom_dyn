...
char nucl_file[200];
sscanf(argv[1], "%s", nucl_file);	// get sequence
//printf("Nucleosome position file: %s\n",nucl_file);
...
int vol_iter;
sscanf(argv[3], "%d", &vol_iter);	// for outputing in different folders

char out_folder[200];
sscanf(argv[4], "%s", out_folder);	// get output folder
//printf("%s\n",out_folder);

int it_check;
sscanf(argv[5], "%d", &it_check);	// for outputing in different folders
...
//char out_folder[] =  "/scratch/orozco/jwalther/sync/Master_Manuel/Manuel_Chromatin_equ_structure/src_test";
...
double **save_en, sh_min, **save_each_en, sed_co;
int len_seg, *ind_linker, nucl_ind, min_ph, *it_vol_olap;
...
it_mc = it_check*it_mcsteps;	// number of MC steps to bring structure to minimum energy before particle inserted
				/* 100000 -> 56mer
				   10000000 -> 1000mer
				   2000000 -> 450mer
				   5000000 -> 750mer
...
want_to_save_en = 0;		// Save energy every int_en_save accepted moves: (1) = yes
detect_indep = 1;		// Save all energy values to detect after what # of structures the structures get independent
...
it_vol_olap = malloc(num_red_mc*sizeof(int));
...
printf("\n");*/
	...
	//vol_olap_start = 0;	
	//printf("Overlap structure %d\n",vol_olap_start);
	it_vol_olap[psi] = vol_olap_start;	// Record vol_olap_start
	...
	if(vol_olap_start == 0){
					...
					write_helpars_in_table(itot_mc,vol_iter,xconf_mc,out_folder,-1);
					printf("psi %d\n",psi);
					//printf("With the current configuration the chromatin fiber is not overlapping\n");
...
iflag=2;
	if(iflag == 1){if(psi%1 == 0){
			...
			write_helpars_in_table(itot_mc,vol_iter,xconf_mc,out_folder,psi);
...
//for(i=0;i<len_all;i++){for(j=0;j<6;j++){printf("%f ",xconf_all[j][i]);} printf(" \n");}
	...
	if(detect_indep == 1){write_indep_in_table(it_each_en,vol_iter,save_each_en,out_folder);}
	//write_vol_olap_in_table(num_red_mc,vol_iter,it_vol_olap,out_folder);
